ActiveAdmin.register_page 'Project weekly hours' do
  ...
  content title: 'Project logs' do
    ...
    if project
      ...
      this_month_weekly_hours = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month)
      ...
      monthly_weekly_hours = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum(:hours)
      monthly_total_amount = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum { |weekly_hour| weekly_hour.total_amount }
      monthly_total_fee = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum { |weekly_hour| weekly_hour.fee }
      monthly_total_external_cost = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum { |weekly_hour| weekly_hour.external_cost }
      monthly_total_net_amount = weekly_hours.where(date: Time.zone.now.beginning_of_month..Time.zone.now.end_of_month).sum { |weekly_hour| weekly_hour.net_amount }
     ...
     panel 'Project Total logs', style: 'font-size: 13px;' do
      ...
      #   table_for weekly_hours.order(date: :desc) do
      #     column('ID') { |wh| wh.id }
      #     column('Hours') { |wh| wh.hours }
      #     column('External Rate') { |wh| wh.external_rate }
      #     column('Created At') { |wh| wh.date }
      ...
      # 
        ...
        table_for this_month_weekly_hours.order(date: :desc) do
